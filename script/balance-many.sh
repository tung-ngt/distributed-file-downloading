...
for i in $(seq 8 $n); do
    java DownloadServiceImpl config/directory.properties random.data 1 para  &> "log/balance-many-download1-$i.log" &
    sleep 1
done

for i in $(seq 2 $n); do
    java DownloadServiceImpl config/directory.properties random2.data 1 para  &> "log/balance-many-download2-$i.log" &
    sleep 1
done
