...
for i in $(seq 1 $sources); do
    java DaemonServiceImpl config/directory.properties "config/daemon$i.properties" &> "log/daemon-para$i.log" &
    sleep 1
done
...
for i in $(seq 1 $sources); do
  ...
  for j in $(seq 1 $n); do
      echo "run $j"
      start=$(date +%s.%N)
      java DownloadServiceImpl config/directory.properties repeating.data download "$i" para &> "log/download-para$i.log"
      end=$(date +%s.%N)
